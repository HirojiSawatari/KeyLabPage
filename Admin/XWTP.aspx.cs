...
public partial class Admin_XWTP : System.Web.UI.Page
{
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        DataControl.Execute("update 新闻图片维护 set 图片1='" + Upload(this.FileUpload1) + "'");
    ...
    {
        //DataControl.Execute("update  set 主题='" + this.txt主题.Text + "',内容='" + txt内容.Text + "'");
        //Page.ClientScript.RegisterStartupScript(this.GetType(), "info", "<script>alert('已经上传！'); </script>");
        Response.Redirect("XWTPList.aspx");
    ...
    {
        if (this.FileUpload5.PostedFile.FileName == "")
        {
            return;
        }
        DataControl.Execute("update 新闻图片维护 set 图片2='" + Upload(this.FileUpload5) + "'");
        Page.ClientScript.RegisterStartupScript(this.GetType(), "info", "<script>alert('已经上传！'); </script>");
    ...
    protected void ImageButton3_Click(object sender, ImageClickEventArgs e)
    {
        ...
        DataControl.Execute("update 新闻图片维护 set 图片3='" + Upload(this.FileUpload3) + "'");
        Page.ClientScript.RegisterStartupScript(this.GetType(), "info", "<script>alert('已经上传！');</script>");
