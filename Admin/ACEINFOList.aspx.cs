...
public partial class Admin_ACEINFOList : System.Web.UI.Page
{
    ...
    private void BindResult()
    {
        ...
        //组合查询条件 
        DataTable dt = DataControl.GetData("select * from [news] ");
        if (dt != null)
        {
            this.gvData.DataKeyNames = new string[] { "编号" };
    ...
    protected void gvData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string strUrl = "ACEINFO.aspx?编号=" + gvData.DataKeys[num]["编号"].ToString();
            PageBase.OpenWebForm(strUrl);
        }
        if (e.CommandName == "Del")
        {
            DataControl.Execute("delete from [news] where 编号=" + gvData.DataKeys[num]["编号"].ToString() + "");
            BindResult();
        }
    }
